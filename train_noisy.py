...
@torch.no_grad()
def evaluate_snr(encoder, decoder, head, dl, device, snr_db: float) -> float:
    encoder.eval(); decoder.eval(); head.eval()
    acc_sum, n = 0.0, 0
    for x, y in dl:
        x, y = x.to(device), y.to(device)
        z = encoder(x)
        z_noisy = awgn(z, snr_db=snr_db, assume_unit_power=True)
...
def train_one_epoch(encoder, decoder, head, dl, optimizer, device,
    ...
    for x, y in dl:
        ...
        z = encoder(x)
        z_noisy = awgn(z, snr_db=snr, assume_unit_power=True)
        feat = decoder(z_noisy)
        logits = head(feat)
...
def main():
    ...
    parser.add_argument("--snr_db", type=float, default=10.0)          # fixed 用
    parser.add_argument("--snr_min", type=float, default=0.0)          # uniform 用
    parser.add_argument("--snr_max", type=float, default=20.0)         # uniform 用
    ...
    print(f"snr_mode={args.snr_mode}, snr_db={args.snr_db}, snr_min={args.snr_min}, snr_max={args.snr_max}")
    ...
    for ep in range(1, args.epochs + 1):
        loss = train_one_epoch(
            ...
            snr_mode=args.snr_mode, snr_db=args.snr_db, snr_min=args.snr_min, snr_max=args.snr_max
        )
        scheduler.step()
        lr_now = scheduler.get_last_lr()[0]

        #SNR=10dB做 best checkpoint
        acc10 = evaluate_snr(encoder, decoder, head, dl_test, device, snr_db=10.0)
    ...
    for s in eval_snrs:
        a = evaluate_snr(encoder, decoder, head, dl_test, device, snr_db=float(s))
